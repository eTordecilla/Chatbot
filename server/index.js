...
const KNOWLEDGE_FILE_PATH = path.resolve(__dirname, '../knowledge/BancoDePreguntasPortalYPedidos.txt');
...
const STOPWORDS = new Set([
  ...
  'en','con','por','para','que','qué','cómo','como','cuando','cuándo',
  'es','son','está','están','se','mi','me','no','si','sí','y','o',
  'a','e','i','u','hay','puedo','puede','tengo','tiene','hacer',
  'ha','han','he','hemos',
]);
...
function loadKnowledge() {
  try {
    ...
    console.log(`✅ Base de conocimiento cargada (${text.length} caracteres, ${blocks.length} entradas)`);
    return { text, blocks };
  } catch (err) {
...
app.get('/api/knowledge-status', (req, res) => {
  try {
    ...
    res.json({ exists: true, path: KNOWLEDGE_FILE_PATH, sizeKB: (stats.size / 1024).toFixed(1), lines, lastModified: stats.mtime });
  } catch (err) {
...
app.post('/api/chat', (req, res) => {
  ...
  if (!knowledge) {
    return res.json({
      reply: 'Lo siento, la base de conocimiento no está disponible en este momento. Por favor contacta al equipo de soporte.',
      replyParts: [{ type: 'text', content: 'Lo siento, la base de conocimiento no está disponible en este momento. Por favor contacta al equipo de soporte.' }],
